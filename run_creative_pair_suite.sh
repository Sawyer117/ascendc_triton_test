...
run_pair() {
  local name=$1
  shift
  run_case "${name}_core" core "$@"
  if [[ "${RUN_FULL_L2NORM}" == "1" ]]; then
    run_case "${name}_full_l2norm" full "$@"
  fi
}
...
if [[ "${RUN_UNSAFE:-0}" == "1" ]]; then
  ...
  run_case varlen_unaligned_1121_core core \
    --case varlen --heads 8 --key-dim 128 --value-dim 128 --chunk-size 64 --tail-topk 16

  run_case varlen_unaligned_1152_core core \
...
if [[ "${RUN_LARGE:-0}" == "1" ]]; then
  run_pair fixed_medium \
    --case medium
  run_pair varlen_many_2048 \
